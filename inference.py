...
import torch
# torch>=2.6 defaults weights_only=True, which breaks PyTorch-Lightning format ckpts
# (v1_face.pth / v2-1_512-ema-pruned.ckpt / the LRPO ControlNet). Keep old behavior.
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="DDIM stochasticity (eta); only affects edm samplers — DDIM uses a fixed internal value.",
    )
    parser.add_argument(
        "--order",
        type=int,
    ...
    parser.add_argument("--llava_bit", type=str, default="16", choices=["16", "8", "4"])
