...
class SpacedSampler(Sampler):
    ...
    def sample_direct_x0(
            ...
            model: ControlLDM,
            device: str,
            t: int,  # index of the sampling timestep, in [0, steps-1]
            steps: int,
            x_size: Tuple[int],
            cond: Dict[str, torch.Tensor],
            uncond: Dict[str, torch.Tensor],
        ...
        # Build the sampling schedule.
        self.make_schedule(steps)
        self.to(device)

        # Generate random noise if no initial noise is provided.
        if x_T is None:
            x_T = torch.randn(x_size, device=device, dtype=torch.float32)
        ...
        # Use t as the index into the actual timestep sequence.
        model_t = torch.full((bs,), timesteps[t], device=device, dtype=torch.long)

        # Compute position_t, the index in the reversed sequence.
        position_t = torch.full((bs,), total_steps - t - 1, device=device, dtype=torch.long)

        # CFG scale for the current timestep.
